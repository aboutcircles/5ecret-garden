...
export function createCirclesSdkProfilesBindings(opts: {
  ...
  function endpointCandidates(path: 'pin' | 'pin-media' | 'raw'): string[] {
    if (!base) return [];
    // Profile pinning service mounts all routes at root (behind Caddy's
    // /profiles reverse-proxy prefix).  Try root first, /api/* as fallback
    // for legacy service configurations.
    if (/\/api$/i.test(base)) {
      return [`${base}/${path}`];
    }
