<script lang="ts">
  ...
  const hasImage: boolean = $derived(
    !!ctx.profile.previewImageUrl &&
      ctx.profile.previewImageUrl.trim().length > 0
  );
  const hasDescription: boolean = $derived(
...
<FlowDecoration>
  <p class="text-sm text-base-content/70 mt-1">
    Confirm details before creating the group.
  </p>
  ...
  <div class="mt-4">
    ...
    {:else}
      <div
        class="w-32 h-32 rounded bg-base-200 flex items-center justify-center text-base-content/50"
      >
        No image
      </div>
    {/if}
