...
>
  ...
  >
    <div class="pointer-events-auto max-w-full">
      ...
      <div
        class="btm-nav btm-nav--float bg-base-100/90 backdrop-blur-md border shadow-lg rounded-full px-2 py-1 max-w-full overflow-hidden"
      >
        {#each items as item (item.link)}
          ...
          {@const baseClasses =
            'inline-flex items-center gap-3 rounded-full px-4 py-2 whitespace-nowrap transition-colors'}
          {@const focusClasses =
