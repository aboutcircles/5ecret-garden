<script lang="ts">
  ...
  import ImageUpload from '$lib/shared/ui/forms/ImageUpload.svelte';
  ...
  function onnewimage(dataUrl: string) {
    ctx.profile.previewImageUrl = dataUrl;
  }
  function oncleared() {
    ctx.profile.previewImageUrl = '';
  }
...
<FlowDecoration>
  ...
  <label class="form-control mt-4">
    <div class="label">
      <span class="label-text"
        >Name <Tooltip content="Enter a name for your group." /></span
      >
    </div>
    <input
      required
    ...
    <div class="h-5 text-xs text-error pt-1">
      {#if showNameInvalid}Invalid name{/if}
    </div>
  </label>
  ...
  <label class="form-control">
    <div class="label">
      <span class="label-text"
        >Description <Tooltip
          content="Brief description of your group."
        /></span
      >
    </div>
    <textarea
