...
/**
 * A single community an avatar has a relationship with, as returned by the
 * `circles_getAvatarCommunities*` RPC methods. `membershipFee` is a percent in
 * [0,100] of the avatar's daily gCRC mint, or `null` when the community's profile
 * declares no fee.
 */
...
export interface AffiliateGroupListResponse {
  /** Sum of `membershipFee` over `communities`; a `null` fee contributes 0. */
  totalFeePercentage: number;
  communities: AffiliateGroupRow[];
}

/**
 * One member avatar in a community's wishlist (signalled intent to join) or
 * confirmed-members list, as returned by the `circles_getCommunityMembers*`
 * RPC methods. The community-centric counterpart to {@link AffiliateGroupRow}.
 */
...
/**
 * The five community RPC methods (`circles_getAvatarCommunities*` /
 * `circles_getCommunityMembers*`) are currently deployed on the staging indexer
 * only. On a production-server RPC the call rejects with `-32601`; callers treat
 * that as "feature unavailable on this server" rather than a hard error, so the
 * Communities tab can show a hint instead of failing.
 */
...
function normalizeListResponse(
  ...
  return {
    ...
    communities: (res?.communities ?? []).map((g) => ({
      communityName: g.communityName ?? null,
      communityAddress: String(g.communityAddress).toLowerCase() as Address,
