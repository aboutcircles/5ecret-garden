...
<p class="font-medium text-sm mt-4">
  {maxAmountCircles >= 0 ? roundToDecimals(maxAmountCircles) : '?'}
  <span class="text-gray-500"
    >/ {roundToDecimals(balanceRow.circles)} CRC
    <Tooltip
      content="The max. amount depends on your trust network and blockchain limits. Try to chunk large transfers if you experience issues."
    /></span
  >
