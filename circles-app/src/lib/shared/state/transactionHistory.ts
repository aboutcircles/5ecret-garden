...
function groupByTransaction(rows: ExtendedTransactionRow[], userAddress: string): GroupedTransaction[] {
  ...
  return Array.from(groups.entries()).map(([hash, events]) => {
    ...
    // Pure mint/burn: counterparty is still zero — show the user's own profile
    // (minting creates YOUR tokens, burning destroys YOUR tokens)
    if (counterparty.toLowerCase() === ZERO_ADDRESS) {
      counterparty = userAddress as Address;
    }
...
async function loadNextPage(): Promise<boolean> {
  ...
  try {
    ...
    // Use SDK's transaction history API — returns PagedResponse with proper pagination
