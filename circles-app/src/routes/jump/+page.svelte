    ...
    {#if destination}
      <div style="background:{T.surface};border:1px solid {T.hairlineSoft};border-radius:12px;padding:16px;display:flex;flex-direction:column;gap:12px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;">
          <a style="display:inline-flex;align-items:center;height:36px;padding:0 18px;border-radius:9999px;border:0;background:{T.primary};color:#fff;text-decoration:none;font-family:{T.fontSans};font-size:13px;font-weight:580;box-shadow:0 4px 12px rgba(88,73,212,0.25);" href={destination} target="_blank" rel="noopener noreferrer">Continue</a>
          <button type="button" style="height:36px;padding:0 18px;border-radius:9999px;border:0;background:transparent;color:{T.inkMuted};cursor:pointer;font-family:{T.fontSans};font-size:13px;" onclick={() => history.back()}>Back</button>
        </div>
      </div>
    {:else}
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <button type="button" style="align-self:flex-start;height:36px;padding:0 18px;border-radius:9999px;border:0;background:transparent;color:{T.inkMuted};cursor:pointer;font-family:{T.fontSans};font-size:13px;" onclick={() => history.back()}>Back</button>
      </div>
    {/if}
  </div>
</div>
